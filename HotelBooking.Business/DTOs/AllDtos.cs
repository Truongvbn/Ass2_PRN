...
public record UpdateRoomDto(int Id, string Name, int RoomTypeId, decimal PricePerNight, int MaxOccupancy, string Description, string ImageUrl, string Amenities, bool IsAvailable); // This one is only instantiated manually in PageModels so positional is fine, but let's be consistent and change it below.
